...
func New(backend Backend, enc encapsulation.Encapsulator, granularity Granularity, hostname string, port int, subnet *net.IPNet, local, cni bool, cniPath, iface string, cleanup bool, cleanUpIface bool, createIface bool, mtu uint, autoMTU bool, resyncPeriod time.Duration, prioritisePrivateAddr, iptablesForwardRule bool, serviceCIDRs []*net.IPNet, logger log.Logger, registerer prometheus.Registerer) (*Mesh, error) {
	...
	if autoMTU {
		mtu = detectMTU(privIface, logger)
	}
...
func (m *Mesh) applyTopology() {
	...
	if m.autoMTU {
		m.mtu = detectMTU(m.privIface, m.logger)
	}
	// Ensure the WireGuard interface has the correct MTU.
	if err := wireguard.SetMTU(m.kiloIface, m.mtu); err != nil {
		level.Error(m.logger).Log("error", fmt.Errorf("failed to set MTU on WireGuard interface: %v", err))
		m.errorCounter.WithLabelValues("apply").Inc()
		return
	}
...
func detectMTU(privIface int, logger log.Logger) uint {
	var baseMTU int
	if privIface != 0 {
		link, err := netlink.LinkByIndex(privIface)
		if err != nil {
			level.Warn(logger).Log("msg", "failed to get private interface for MTU detection, falling back to default", "error", err)
			baseMTU = 0
		} else {
			baseMTU = link.Attrs().MTU
			level.Info(logger).Log("msg", fmt.Sprintf("detected underlay MTU %d on interface %s", baseMTU, link.Attrs().Name))
		}
	}
	if baseMTU == 0 {
		iface, err := defaultInterface()
		if err != nil {
			level.Warn(logger).Log("msg", "failed to get default interface for MTU detection, using default MTU", "error", err)
			return wireguard.DefaultMTU
		}
		link, err := netlink.LinkByIndex(iface.Index)
		if err != nil {
			level.Warn(logger).Log("msg", "failed to get default interface link for MTU detection, using default MTU", "error", err)
			return wireguard.DefaultMTU
		}
		baseMTU = link.Attrs().MTU
		level.Info(logger).Log("msg", fmt.Sprintf("detected underlay MTU %d on default interface %s", baseMTU, link.Attrs().Name))
	}
	mtu := uint(baseMTU) - wireguard.WireGuardOverhead
	level.Info(logger).Log("msg", fmt.Sprintf("auto-detected WireGuard MTU: %d (underlay %d - overhead %d)", mtu, baseMTU, wireguard.WireGuardOverhead))
	return mtu
}
